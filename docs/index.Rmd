...
### Summary Information
This is a summary of our analysis. In our project, we find the zip code with most breast cancers is *summary_info$zipmostBcancer*. We found the highest survival months in T1 is *summary_info$survivalTstage1*. We found the highest survival months in T2 is *summary_info$survivalTstage2*. We found the highest survival months in T3 is
*summary_info$survivalTstage3* For female in marriage status, the most occurrence of T stage is *summary_info$maritalrelationM*. For female in divorce status, the most occurrence of T stage is
*summary_info$maritalrelationD* For female in Single status, the most occurrence of T stage is *summary_info$maritalrelationS*, it might because the sample in single group is too small. 
For female in widow status, the most occurrence of T stage is *summary_info$maritalrelationW* 
...
```
The plots is used in respond of question one. The plot is used to examine the relationship between breast cancer and location. We calculate the ratio of breast cancer in all cancers as the breast cancer indicator and use zip codes in the dataset as the indicator of the location
Because location is categorical and ratio is numerical, we adopt barplot. According to the graph, it does show that ratio varies with zip code. It might implies that location could contribute to breast cancer.
...
We use this plot to examine question 3, that is if survival rates of relate to Marital status.
